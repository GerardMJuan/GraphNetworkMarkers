...
def par_extract_values(row, subj_dir, cortical):
    ...
    for graph, name, pairs in zip(
        list_of_graphs, list_of_names, list_of_pairs
    ):
        df_G.at[idx_G, f"{name}_avg_spl"] = global_efficiency_weighted(
            deepcopy(graph), deepcopy(pairs)
        )
        df_G.at[idx_G, f"{name}_avg_eff"] = global_efficiency_weighted_inverse(
            deepcopy(graph), deepcopy(pairs)
        )
...
@click.option(
    "--njobs", required=True, type=click.STRING, help="number of jobs"
)
@click.option("--cortical", is_flag=True, help="use only cortical values.")
@click.argument("subj_dir")
def compute_SC_values(
    subj_dir, total_csv, pip_csv, out_csv_prefix, njobs, cortical
):
    """
    Compute SC values
    """
    ...
    results = Parallel(n_jobs=njobs, backend="threading")(
        delayed(par_extract_values)(row, subj_dir, cortical)
